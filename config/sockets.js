...
module.exports.sockets = {
    onConnect: function(session, socket) {
        if(session && session.user && gameState.players[session.character.id]) {
            console.log('blanking payload')
            gameState.players[session.character.id].lastPayload = {};
        }
    },
    ...
    if(session && session.user && gameState.players[session.character.id]) {
        Hooks.handle('CharacterDisconnected', {character:gameState.players[session.character.id]}, function() {
            gameState.players[session.character.id].saved = Date.now();
            UserService.storeCharacter(gameState.players[session.character.id], function() {
                delete gameState.players[session.character.id];
            });
        });
    }
},
